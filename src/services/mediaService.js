...
export async function createNewMedia(mediaData) {
    ...
    validaMedias(mediaData) //chama função para validar os dados
...
export function validaMedias(data) {
    ...
    if (!data.type || typeof data.type !== 'string' || data.genre.trim() === '') { //valida type

        throw new Error("Por favor, o campo 'Tipo' não pode estar vazio")

    }

    if (data.releaseYear === undefined || data.releaseYear === null) { //valida releaseYear
